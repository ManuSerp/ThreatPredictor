...
def main():
    ...
    cluster_model = KMeanModel(n_clusters=10, random_state=0)
    ...
    data_out = cluster_model.train(data)
    predict_labels = cluster_model.predict(data)

    cluster_model.evaluating_clustering_performance(data_out,predict_labels)

    # Cluster label

    print("==== Cluster label begining ====")
    cluster_label = ClusterLabel(predict_labels,label,cluster_model.n_clusters)
